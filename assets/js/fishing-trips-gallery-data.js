...
(() => {
  ...
  /*
   * Existing website photographs remain under Documentation.
   */
  const documentationPhotos = [
    ...
    ([src, tag, title, description, layout]) => ({
      src,
  ...
  /*
   * Images inside the renamed folder:
   * friends-and-catches/gal (1).png through gal (31).png
   */
  const friendsAndCatchesPhotos = Array.from({ length: 31 }, (_, index) => {
    const number = index + 1;
    let layout = "";

    if (number === 1 || number === 16 || number === 31) {
      layout = "feature";
    } else if (number % 7 === 0) {
      layout = "wide";
    } else if (number % 5 === 0) {
    ...
    return {
      ...
      tag: "Friends & Catches",
      title: `Fishing trip memory ${String(number).padStart(2, "0")}`,
      description: "Friends, guests, and catches from Mesopelagic Anglers.",
      layout
    };
  });

  friendsAndCatchesPhotos.push({
  ...
  const createCard = (photo, index) => {
    ...
    button.dataset.galleryAlt = `${photo.title}, Mesopelagic Anglers fishing trip photograph`;
  ...
  const allPhotos = [...documentationPhotos, ...friendsAndCatchesPhotos];
